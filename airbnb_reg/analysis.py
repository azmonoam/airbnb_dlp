...
def count_image_num_anf_plot(args, color, test_only = True):
    ...
    ax1.axis('equal')
    ax1.set_title('Most important photo order distribution')
...
def plot_loss_per_city_and_base_line(args,avg_loss_per_city, color):
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(x - width, avg_loss_per_city['loss_in_usd'], width, label='Model prediction loss', color = color[3])
    rects2 = ax.bar(x, avg_loss_per_city['median_baseline_in_usd'], width, label='Median prediction loss', color = color[5])
    rects3 = ax.bar(x + width, avg_loss_per_city['mean_baseline_in_usd'], width, label='Mean prediction loss', color = color[1])
    ax.set_ylabel('Loss in USD $')
    ax.set_title('MSE Loss in USD per city')
    ax.set_xticks(x)
...
def plot_room_type_for_important_photo_vs_baseline(att_df_rooms, color):
    ...
    labels = I0_data['I0_room_type'].str.capitalize().tolist()
    ...
    rects1 = ax.bar(x - width / 2, I_importat_data['id'], width, label='Chosen by model',  color=color[0])
    rects2 = ax.bar(x + width / 2, I0_data['id'], width, label='Chosen by user',  color=color[2])
    ax.set_ylabel('number of listings')
    ax.set_title('Room type distribution of the most important photo')
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.legend()
    autolabel(ax, rects1)
...
def get_top_and_bottom_apt(args, num_photos, pred = True):
    predictions_path = args.base_path + args.results_path + f"/losses/predictions_{args.job_id.split('_lr')[0]}.csv"
    predictions = pd.read_csv(predictions_path)
    if pred:
        ...
        sorted_test_pred = predictions.loc[predictions['type'] == 'test'].sort_values(by='price')
        bottom = sorted_test_pred .iloc[:num_photos][['id', 'price']].reset_index()
        top = sorted_test_pred .iloc[-num_photos:][['id', 'price']].reset_index().sort_values(by='price', ascending=False)
    return top, bottom
...
def plot_radical_apt_most_imp_photo(args, paths, num_photos, kind):
    ...
    for i in range(num_photos):
        for j in range(num_photos):
            ...
            axarr[i, j].axis('off')
            k += 1
    plt.suptitle(f'Most important photo of listings with the {kind}')
    plt.tight_layout()
...
def plot_i0_vs_most_impd(paths, num_apt, kind):
    f, axarr = plt.subplots(num_apt, 2, figsize=(4,6))
    for i in range(num_apt):
    ...
    plt.suptitle(f'Most important photo')
    plt.tight_layout()
...
if __name__ == '__main__':
    ...
    plot_att_diffs_photos(top_paths, 5, 'Listings with the lowest relative weight of the most important photo')
    plot_att_diffs_photos(bottom_paths, 5, 'Listings with the highest relative weight of the most important photo')
